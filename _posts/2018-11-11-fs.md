...
Kata employs the virtual machine isolation, Gvisor the xxx sandbox [TODO], and Nabla the libOS+Seccomp based attack surface minimization.
...
Nabla's [philosophy](https://nabla-containers.github.io/) of limiting access to the host kernel means that a container's filesystem access would be handled inside the LibOS and never reach the host kernel. 
...
However, it does not seem like a security feature, as [indicated](https://github.com/google/gvisor/blob/master/pkg/sentry/syscalls/linux/linux64.go#L239-L250) by Sentry's lack of support for the syscall--"`// Removexattr, requires filesystem support`".
...
In this blog, although we chose the filesystem interface to highlight Nabla's *touch-point* (attack surface) minimization philosophy, the same is applicable to other components as well, including the virtualization interface itself! [TODO: add link to virt/qemu bug].
